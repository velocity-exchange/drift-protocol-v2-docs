...
<Callout type="info">
Not to be confused with an [Isolated Insurance Fund](/protocol/insurance-fund) (a per-market insurance backstop) or [isolated positions](/protocol/trading/margin) (per-position perp margin). Isolated pools are segregated collateral pools.
</Callout>
...
3. **Open a USDT Borrow Position**: Simultaneously borrow USDT using the same JLP collateral.
4. **Risk Management**:
    - Both positions rely on the same JLP collateral.
    - If JLP's value drops significantly, both the SOL and USDT positions may face liquidation.
</Callout>

<Callout type="info">
### Example - Isolated Pool
1. **Choose an Isolated Pool**: Select a JLP/USDT pool 
2. **Deposit Collateral**: Deposit 1000 JLP into the pool.
3. **Open a Leveraged Position**: Borrow USDT and swap into JLP. Borrow more USDT based on the new deposit of JLP and swap into JLP, creating a leveraged exposure to JLP. You can now earn higher yield on your initial deposit of 1000 JLP
4. **Risk Management**:
